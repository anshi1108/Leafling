...
// Connect to MongoDB
mongoose
  .connect("mongodb+srv://sadneyasam05:root@cluster1.zxxgt.mongodb.net/", {
...
app.post("/api/users/info", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "An error occurred while updating information" });
  }
});
...
const Product = mongoose.model("product", {
  ...
  uploader: {
    type: String,
    required: true, // Add this field to store the uploader's name
  },
});
...
app.post("/addproduct", async (req, res) => {
  const product = new Product({
    ...
    uploader: req.body.uploader, // Add the uploader's username here
  });
  ...
  try {
    ...
    console.log("Saved");
    res.json({
      success: true, // Fix typo: 'succes' to 'success'
      name: req.body.name,
    });
  } catch (error) {
